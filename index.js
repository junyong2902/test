...
//회원 정보 수정
app.put('/Community_Member_update/:member_id', (req, res) => {
    var member_id=req.body.member_id;
    var member_pd=req.body.member_pd;
    var member_name=req.body.member_name;
    var member_email=req.body.member_email;
    ...
    connection.query('UPDATE Community_Member SET member_pd="?", member_name="?", member_email="?", member_phnumber="?" WHERE member_id=' + req.params.member_id,
    [member_pd, member_name, member_email, member_phnumber], (error, results, fields) => {
     if (error) throw error;
     res.json({ok:"club_signup"});
    });
});
...
// 회원 탈퇴
app.delete('/Community_Member_delete/:member_id', (req, res) => {
  connection.query('DELETE FROM Community_Member WHERE member_id=\'' + req.params.member_id + '\'', (error, rows) => {
    if (error) throw error;
    console.log('member info is: ', rows);
    res.send(rows);
  });
